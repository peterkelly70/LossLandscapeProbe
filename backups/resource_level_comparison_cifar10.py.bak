#!/usr/bin/env python3
"""
Resource Level Comparison for CIFAR-10

This script compares the effectiveness of meta-models trained at different resource levels
(0.1, 0.2, 0.3, 0.4) on the CIFAR-10 dataset. For each resource level, it:
...
def get_cifar10_loaders(subset_fraction=1.0):
...
def create_model(config):
    ...
    return SimpleCNN(num_channels=num_channels, dropout_rate=dropout_rate)
...
def run_experiment():
    ...
    for resource_level in RESOURCE_LEVELS:
        ...
        def dataset_fn(fraction=1.0):
            return get_cifar10_loaders(subset_fraction=fraction * resource_level)
        ...
        with open(os.path.join(RESULTS_DIR, f'cifar10_resource_level_{resource_level}_{DATE_STR}.json'), 'w') as f:
    ...
    # Save summary to CSV
    csv_path = os.path.join(RESULTS_DIR, f'cifar10_resource_level_comparison_{DATE_STR}.csv')
