...
def main():
    ...
    # Get full dataset
    train_loader, val_loader = get_cifar10_loaders(data_fraction=1.0)
    ...
    for epoch in range(epochs):
        ...
        logger.info(f"Epoch {epoch+1}/{epochs} - Loss: {train_loss:.4f}, Test Acc: {val_acc:.4f}")
    
    # Save the model
    torch.save(model.state_dict(), 'cifar10_multisamplesize_trained.pth')
    ...
    # Save results
    torch.save(results, 'cifar10_multisamplesize_results.pth')
    # Also save with the standard name for backward compatibility
    torch.save(results, 'cifar10_results.pth')
    logger.info(f"Model saved to cifar10_multisamplesize_trained.pth")
    logger.info(f"Results saved to cifar10_multisamplesize_results.pth and cifar10_results.pth")
    
    # Save training log
    with open('cifar10_training.log', 'w') as f:
        for epoch in range(epochs):
            f.write(f"Epoch {epoch+1}/{epochs} - Loss: {train_losses[epoch]:.4f}, Test Acc: {val_accs[epoch]:.4f}\n")
