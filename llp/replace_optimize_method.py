#!/usr/bin/env python3
"""
Script to replace the optimize_hyperparameters method in the CIFAR meta-model.
This is a direct approach that simply replaces the method with our improved version.
"""
import os
import re

def replace_optimize_method():
    """Replace the optimize_hyperparameters method in the CIFAR meta-model."""
    # Define the path to the CIFAR meta-model file
    meta_model_path = os.path.join('llp', 'cifar_meta_model.py')
    
    # Define the improved method with proper progress bars
    improved_method = """    def optimize_hyperparameters(self) -> Dict[str, Any]:
        \"\"\"
        ...
        # Define progress bar format once to avoid duplication
    ...
    with open(meta_model_path, 'r') as f:
        content = f.read()
    
    # Define a pattern to match the optimize_hyperparameters method
    pattern = r'    def optimize_hyperparameters\(self\).*?(?=    def |$)'
    
    # Replace the method with our improved version
    new_content = re.sub(pattern, improved_method, content, flags=re.DOTALL)
    
    # Write the modified content back to the file
    with open(meta_model_path, 'w') as f:
        f.write(new_content)
    
    print("Successfully replaced the optimize_hyperparameters method!")

if __name__ == "__main__":
    # Create a backup of the original file
    import shutil
    meta_model_path = os.path.join('llp', 'cifar_meta_model.py')
    backup_path = os.path.join('llp', 'cifar_meta_model.py.bak')
    
    if not os.path.exists(backup_path):
        print(f"Creating backup of {meta_model_path} to {backup_path}...")
        shutil.copy2(meta_model_path, backup_path)
    
    # Replace the optimize_hyperparameters method
    replace_optimize_method()
    
    print("Done! The CIFAR meta-model now has improved progress bars.")
