...
class CIFARReporter:
    ...
    def generate_test_report(self):
        ...
        cm_path = self.run_dir / f"{self.dataset}_{self.sample_size}_confusion_matrix.png"
        if cm_path.exists():
            try:
                with open(cm_path, 'rb') as f:
                    confusion_matrix_img = base64.b64encode(f.read()).decode('utf-8')
            except Exception as e:
                logger.error(f"Error loading confusion matrix image: {e}")
